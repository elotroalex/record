...
Kraus misses an opportunity, though, when she diagnoses the dearth of conjecture in contemporary textual scholarship and archival practices. For Kraus, the lack comes from undue emphasis on the "wealth of scholarship promoting the text's bibliographic or iconic codes," generously averring that "despite the virtues of such visual approaches to textuality (and there are many), conjectural criticism has been hamstrung by their success." Just like Eureka moments can "open the floodgates"—mechanical or otherwise—an *autographic* textuality that is allowed to, indeed must return unimpeached from *allography* can only increase the bounty. Not only can a text never escape its embodiment and must return in some bibliographic form if the output of a computational approach is to be textual, it does so with a bibliographic code determined by *allographic* processes.[^b7]
