...
We can call "[Toussaint King]({{site.baseurl}}/editions/sdw-king/)" a minimal diplomatic edition of the combination of `P1` and `P2` in that order, and the revisions made on the combined text. In other words, pages 23-40 followed by pages 2-22. The diplomatic approach will allow us to study more closely the transposition in time and order within the drama, and the allographic transformation of bibliographic codes. In a very real sense, a speculative diplomatic lays bare what a clean reading text hides: multiple temporalities, multiple stages, revealed in the act of opening up a new one. Meant for editors, a diplomatic turns the reader into one. Two temporalities stand out in my reading: one that belongs to `S2` when the text stood mostly unrevised, and another to `S5+` when Césaire made most of the substantial handwritten revisions.
...
In most of these cases, the rules take on the role of an editor in search of patterns to normalize and render, but in one case the rule addresses a unique occurrence, an exception. On `P2.9` (8), at the top, the characters "Crép" are overtyped with the letter x to make way for a parentheses. I created a rule to deal with just this one case, when I could've quickly edited it at the end of the algorithmic transformation. I wanted to stay true to form, though. Because I'm reducing additions and deletions and normalizing some, a monstruous decision in some quarters, I wanted to remove the double deletion. Rather than finding and replacing the sequence "Crép", though, I did it in such a way that imagines the exception from a bibliographical point of view:
