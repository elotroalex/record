...
The rules select the first 22 pages of the current pagination, which form the basis for this speculative stage. The edition I wanted to create for this stage could be called a reading diplomatic. My choices combine some normalization (œ for oe, inconsistent spacing, missing parentheses) with a reduced representation of erasures and additions. In this sense, I instructed the machine to ignore the 'sic' and the 'orig' in the original TEI, and privilege my corrections and normalizations, also documented in the TEI file. In cases of unclear characters or words in the original, I instructed the machine to select only those conjectures that were marked with a 50% plus level of certainty in the TEI—the house odds.

In a few cases, the rules take on the role of an editor in search of patterns to normalize, but in one case that rule addresses a unique occurrence, an exception. On page 8, at the top, the characters "Crép" are overtyped with the letter x to make way for a parentheses. I created a rule to deal with just this one case, when I could've quickly edited it at the end of the process. Because I'm balancing normalization with diplomatic, a monstruous decision in some quarters, I wanted to simply remove these characters. Rather than finding and replacing the sequence "Crép", though, I did it in such a way that imagines the exception from a bibliographical point of view:
