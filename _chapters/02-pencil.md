...
Kraus foregoes an opportunity opened by her own arguments, though, in her diagnosis of the root causes of the dearth of conjecture in contemporary textual scholarship and archival practices. For Kraus, the lack comes from the relatively recent "wealth of scholarship promoting the text's bibliographic or iconic codes," generously averring that "despite the virtues of such visual approaches to textuality (and there are many), conjectural criticism has been hamstrung by their success." Just like Eureka moments can "open the floodgates" for Kraus—mechanical or otherwise—an *autographic* textuality that is allowed to, indeed must return unimpeached from *allography* can only increase the bounty. Not only can a text never escape its embodiment and must return in some bibliographic form if the output of a computational approach is to be textual, it does so with a bibliographic code determined by *allographic* processes—aligning with the rest of Kraus' argument.[^b7]

--- mark sample paragraph ---
--- keep the rules, add the bibliographic code --- 
...
[^b7]: CSS and LaTeX are both well-known documentations of this phenomenon. Last week, regaling over some tequila mockingbirds with fellow traveler Dennis Tenen, the subject turned to the slippery back and forth between the analog and the digital. I noticed that no digital humanist we knew of had yet tried to data mine CSS files for web pages. I was only half-joking. 
